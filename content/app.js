...
function init() {
    ...
    for(var playerNum = 0; playerNum < 2; playerNum++) {
        ...
        document.getElementById('name-' + playerNum).textContent = 'Player ' + playerNum;
        document.querySelector('.player-' + playerNum + '-panel').classList.remove('winner');
...
function rollDie() {
    if (gamePlaying) {
        ...
        if (dice !== 1) {
            ...
            if(autoHold && roundScore + scores[activePlayer] >= scoreToWin) {
                hold();
            }
        } else {
            nextPlayer();
...
function nextPlayer() {
    ...
    for (var playerNum = 0; playerNum < 2; playerNum++) {
        document.getElementById('current-' + playerNum).textContent = '0';
        document.querySelector('.player-' + playerNum + '-panel').classList.toggle('active');
    }
